...
public class TransitionManager : MonoBehaviour
{
    ...
    private void UpdateVisibleObjects()
    {
		...
		if (!InAR) {
			...
			GameObject.Find ("Character").transform.position = new Vector3 (36f, 7.4f, -60f);
			GameObject.Find ("AudioPlayer").GetComponent<AudioSource> ().Play ();
			modelCenter.GetComponent<VRModelActivator> ().ActivateModel ();
		} else {
			foreach (Camera cam in VRCameras)
				cam.clearFlags = CameraClearFlags.SolidColor;
			//VRCameras[0].transform.parent.parent.GetComponent<CharacterController> ().enabled = false;
